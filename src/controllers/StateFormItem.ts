...
const handleStr = this.itemHasJson.handle
const handle = this.getHandlePropName(handleStr)

if (!handle) {
  err('Invalid handle syntax. Expected "<event> : <property>" format.')
  return
}

const handleCallback = getVal(window, handle.callbackName)
...
this.assignCallback(handle.event, handleCallback)
} // END of method
