 ...
 * @param strFn 
 */
function parseStrFn(strFn: string|number) {
  if (typeof strFn === 'number') {
    return strFn
  }
  ...
  for (var i = 1; i < strFnPieces.length; i++) {
    ...
    parsed.push(parsedValue !== NaN ? parsedValue : arg)
  }
  return parsed
}

function callFn(theme: Theme, fnName: string, args: (string|number)[]) {
  const candidate = getVal(theme, fnName)
...
export function applyFn(theme: Theme, styles: any) {
  for (const prop in styles) {
    const value = styles[prop]
    const parsed = parseStrFn(value)
    ...
    case 'object':
      if (Array.isArray(parsed)) {
        const fnName = parsed.shift() as string
        styles[prop] = callFn(theme, fnName, parsed)
      }
      break
    }
  }
